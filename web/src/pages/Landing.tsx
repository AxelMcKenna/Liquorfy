  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-primary text-white">
        <div className="pointer-events-none absolute -top-40 left-1/2 h-[28rem] w-[42rem] -translate-x-1/2 rounded-full bg-white/15 blur-3xl" />
        ...
        <div className="max-w-5xl mx-auto px-6 py-32 text-center">
          <motion.h1
            ...
            className="text-6xl md:text-7xl font-semibold mb-6 tracking-tight leading-[1.1]"
          >
            Find Better Prices
          </motion.h1>
          <motion.p
            ...
            className="text-2xl mb-16 max-w-xl mx-auto text-white/85 font-normal"
          >
          ...
          >
            <div className="relative group">
              <Search className="absolute left-6 top-1/2 transform -translate-y-1/2 h-5 w-5 text-slate-500 group-focus-within:text-primary transition-colors" />
              <Input
                placeholder="Search for beer, wine, or spirits"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                className="pl-16 pr-32 py-8 text-lg rounded-2xl border-2 border-transparent bg-white text-foreground focus:border-white/80 focus:ring-0 transition-all shadow-lg shadow-black/10"
              />
              <Button
                onClick={handleSearch}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-white text-primary hover:bg-white/90 px-8 py-5 text-base font-medium rounded-xl shadow-none"
              >
                Search
              </Button>
            </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {location && (
              <div className="space-y-8">
                ...
                <div ref={mapRef} className="rounded-2xl overflow-hidden border border-border">
                  {shouldLoadMap ? (
                    <LazyStoreMap
                    ...
                    <StoreMapSkeleton className="h-[500px] rounded-2xl border-none" />
                  )}
                </div>
              </div>
            )}
